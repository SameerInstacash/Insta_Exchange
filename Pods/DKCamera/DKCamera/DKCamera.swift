...
open class DKCamera: UIViewController, AVCaptureMetadataOutputObjectsDelegate {
    ...
    open override func viewWillAppear(_ animated: Bool) {
        ...
        if !self.captureSession.isRunning {
            ...
            //* MARK: Add this code on 4/8/23 due to crash on Main Thread
            // Create a background queue
            let sessionQueue = DispatchQueue(label: "com.DKCamera.sessionQueue", qos: .userInitiated)

            // Start the AVCaptureSession on the background thread
            sessionQueue.async {
                self.captureSession.startRunning()
                
                // Any other related configuration or processing can also be done here.
                
                // If you need to update the UI based on the session state, do it on the main thread.
